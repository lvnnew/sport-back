...
export const getAppRefreshTokensService = (ctx: Context) => {
  ...
  const findOne = async (
    ...
    return ctx.prisma.appRefreshToken.findFirst(toPrismaRequest(params, {noId: true}));
  };
  ...
  const create = async (
    ...
    await ctx.prisma.appRefreshToken.update({
      where: {id: result.id},
      data: {
        search: [
          ...R
    ...
    await afterCreate(ctx, result as AppRefreshToken);
