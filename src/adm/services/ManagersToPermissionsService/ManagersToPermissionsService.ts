...
export const getManagersToPermissionsService = (ctx: Context) => {
  ...
  const findOne = async (
    ...
    return ctx.prisma.managersToPermission.findFirst(toPrismaRequest(await changeListFilter(params, ctx), {noId: false}));
  };
  ...
  const count = async (
    ...
    return ctx.prisma.managersToPermission.count(toPrismaTotalRequest(await changeListFilter(params, ctx)));
  };
  ...
  const update = async (
    ...
    await Promise.all([
      afterUpdate(ctx, result as ManagersToPermission),
    ]);
