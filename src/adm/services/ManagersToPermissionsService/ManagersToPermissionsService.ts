...
export interface BaseManagersToPermissionsMethods {
  ...
  update: ({id, ...rest}: MutationUpdateManagersToPermissionArgsWithoutAutodefinable, byUser?: boolean) =>
    Promise<ManagersToPermission>;
  upsert: (
    data: PartialFieldsInRecord<MutationUpdateManagersToPermissionArgsWithoutAutodefinable, 'id'>,
    byUser?: boolean,
  ) =>
    Promise<ManagersToPermission>;
  upsertAdvanced: (
...
export const getManagersToPermissionsService = (ctx: Context) => {
  ...
  const create = async (
    ...
    await runHooks.afterCreate(ctx, result as ManagersToPermission);
  ...
  const upsert = async (
    ...
    // augment data by fields from db
    const augmented: StrictUpdateManagersToPermissionArgs =
      R.mergeLeft(augmentedByDefault, dbVersion || {} as ManagersToPermission);
