...
export interface BasePermissionsMethods {
  ...
  upsertAdvansed: (filter: PermissionFilter, data: MutationCreatePermissionArgs) => Promise<Permission>;
...
export const getPermissionsService = (getCtx: () => Context) => {
  ...
  const upsertAdvansed = async (filter: PermissionFilter, data: MutationCreatePermissionArgs): Promise<Permission> => {
    if (!getCtx()) {
      throw new Error('Context is not initialised');
    }
