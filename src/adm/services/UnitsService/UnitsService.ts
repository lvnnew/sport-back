...
export const getUnitsService = (ctx: Context) => {
  ...
  const augmentByDefault = async <T>(currentData: Record<string, any>): Promise<T & AutodefinableUnitPart> => currentData as T;
  ...
  const createMany = async (
    ...
    // Augment with default field
    const augmentedByDefault = await Promise.all(clearedData.map(el => augmentByDefault(el))) as ReliableUnitCreateUserInput[];
